#include <CLI/App.hpp>
#include <CLI/Config.hpp> // NOLINT misc-include-cleaner // needed for linking
#include <CLI/Formatter.hpp> // NOLINT misc-include-cleaner // needed for linking
#include <fmt/core.h>
...
// configuration step. It creates a namespace called `cpp_weekly_game_project`. You can modify
...
void consequence_game()
{
    ...
    const auto make_buttons = [&] {
        ...
        for (std::size_t cur_x = 0; cur_x < game_board.width; ++cur_x)
        {
            ...
            {
                buttons.push_back(
                    ftxui::Button(&game_board.get_string(cur_x, cur_y),
                                  [=, &game_board] {
...
{
    Bitmap(std::size_t // NOLINT same typed parameters adjacent to each other
               width,
           std::size_t // NOLINT same typed parameters adjacent to each other
               height)
    ...
    void Render(ftxui::Screen &screen) override
    {
        for (std::size_t cur_x = 0; cur_x < width_; ++cur_x)
        {
            for (std::size_t cur_y = 0; cur_y < height_ / 2; ++cur_y)
            {
                ...
                pixel.background_color = ftxui::Color{top_color.R.get(),
                                                      top_color.G.get(),
                                                      top_color.B.get()};
                pixel.foreground_color = ftxui::Color{bottom_color.R.get(),
                                                      bottom_color.G.get(),
                                                      bottom_color.B.get()};
            }
        }
...
int main(int argc, const char **argv)
{
    try
    {
        CLI::App app{
            fmt::format("{} version {}",
                        cpp_weekly_game_project::cmake::project_name,
                        cpp_weekly_game_project::cmake::project_version)};
